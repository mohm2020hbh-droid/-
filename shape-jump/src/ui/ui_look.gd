...
## the scenes' own reds; World 02 gets a black-and-white UI theme, and every
## colored label is remapped by role (accent, text, muted) to the palette.
...
static func apply(root: Node, look: StringName) -> void:
	for node in root.find_children("*", "Control", true, false):
		...
		if control.get_parent() is CanvasLayer or control.get_parent() == root:
			control.theme = MONO_THEME if look == &"mono" else null
		if control is Label and control.has_theme_color_override(&"font_color"):
			...
			control.add_theme_color_override(&"font_color", original if look != &"mono" else _mono(original))
		control.queue_redraw()


## The monochrome stand-in for a World 01 UI color, by its role.
