...
// 1 — a fresh profile: one level open, one locked, nothing in the purse.
{
  ...
  check('the level select lists every level', cards === 30, `${cards} cards`);
  const locked = await page.locator('#menu .card[disabled]').count();
  check('locked levels cannot be tapped', locked === 29, `${locked} disabled`);
  ...
  check('the level select is split into worlds', worlds === 5, `${worlds} banners`);
  const names = await page.locator('#menu .world .wt').allTextContents();
  check('and each world is named',
    names.join('/') === 'NEON CITY/NEON DESERT/THE ABYSS/CLOCKWORK/OVERGROWTH', names.join('/'));
  const far = await page.locator('#menu .world.far').count();
  check('the worlds you have not reached read as far off', far === 4, `${far} dimmed`);
