...
/** World 1's finale sits exactly on the floor, so this level is the one that
 *  defines it. It may equal 0.075s; it may never go under. */
private const val FAIR_FLOOR = 0.075

class Level6Test {
    private val level = Level6.build()
    ...
    @Test fun `a finale does not teach`() {
        val opening = report.jumps.filter { it.percent < 12 }
        assertTrue(opening.size >= 3,
            "world 1's last level ambles for its first eighth: only ${opening.size} jumps")
    }

    @Test fun `both boosts are compulsory`() {
        assertTrue(report.boosts >= 2, "only ${report.boosts} boost(s) forced in the finale")
    }
    ...
    @Test fun `report`() {
        println("LEVEL 6 '${level.name}' -> solvable=${report.solvable} taps=${report.taps} " +
